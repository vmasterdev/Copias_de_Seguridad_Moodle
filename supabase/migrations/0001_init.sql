...
create table if not exists public.backups (
  ...
  category text not null check (category in ('ESCRIBA','CRIBA','INNOVAME','DISTANCIA 4.0')),
  program_code text not null references public.programs(code) on delete restrict,
...
create index if not exists backups_name_trgm on public.backups using gin (file_name gin_trgm_ops);
create index if not exists backups_cat_idx on public.backups (category);
create index if not exists backups_prog_idx on public.backups (program_code);
create index if not exists backups_rect_idx on public.backups (rectoria);
create index if not exists backups_year_idx on public.backups (year);

alter table public.programs enable row level security;
alter table public.backups enable row level security;

create policy "Leer programas" on public.programs for select using (true);
create policy "Gestionar programas" on public.programs for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');

create policy "Leer backups" on public.backups for select using (true);
create policy "Gestionar backups" on public.backups for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
